...
Polyhedron::~Polyhedron()
{
    ...
    this->deallocateInscribedEllipsoidSolver();
...
std::vector<int> Polyhedron::removeConstraints()
{
    ...
    Eigen::MatrixXd ABnew = removeDuplicateRows(AB,1e-5);
...
std::vector<int> Polyhedron::removeConstraints(const Eigen::MatrixXd& A, const Eigen::VectorXd& b, Eigen::MatrixXd& Aout, Eigen::VectorXd& bout)
{
    ...
    Variable::t x = M->variable("x", A.cols(), Domain::inRange(bounds.first, bounds.second));
    Constraint::t con;
    Aout = A;
    bout = b;
...
void Polyhedron::deallocateInscribedEllipsoidSolver()
{
    if (solverInscribedEllipsoidAllocated)
    {
        this->solverInscribedEllipsoidAllocated = false;
        this->MInscribedEllipsoid->dispose();
    }
